...
(async () => {
  ...
  Ξ NOTE  : 116 111 111 108 115 105 103  118 51 
    ...
    try {
        const { username, password, url, perExec, delayTime, inputMessage } = await inquirer.prompt(questions);
        ...
        for (let i = 0; i < likers.length; i++) {
            ...
            if (i < likers.length - 1) print(`Current Account: (${login.username}) » Delay: ${perExec}/${delayTime}ms \n`, "wait", true);
